...
\comment{the green line for white is not visible. chaneg it to black. also, can you make the lines thicker? finally, the legends are wrong: LM is rpeated 3 tiems}}
        \vspace{-0.1in}
        \label{fig:initial_evaluation_2}
\end{figure*}

To accurately measure the phone power draw in all our experiments, \eg
in displaying a static image, we directly connect each of the three
...
\ie \dcomment{$\sum P_i$ => } \dacomment{$\sum_{i=0}^{n}{P_i}$} in Eqn.~1,
...
a maximum difference of 92\%, 66\% and 43\% at
0 (14), 0.04 (59) and 0.01 (25) linear RGB values (sRGB value) for the 3 phones respectively.
...
% widens with
varies across the color intensity, reaching
55\%, 45\% and 26\% at 0.05 (64), 0.05 (67) and 0.01 (27) linear RGB values (sRGB value)
% at linear RGB value 1.0
for the three phones, respectively.
Simillar gaps are also observed for yellow 
60\%, 16\% and 18\% at 0.05 (68), 0.10 (91) and 0.02 (43) linear RGB values (sRGB value)
and for cyan
44\%, 17\% and 21\% at 0.0 (19), 0.23 (130) and 0.01 (34) linear RGB values (sRGB value)
respectively for the three phones.
These results show that
%
{\em subpixel powers are not additive.} The likely reason can be that the
  subpixels utilize common driving circuits within the display
  hardware module.
ADDED}
...
our measured value for the former, 89mA, \comment{The numbers here are all wrong??? FIXED}
is 1.26X
lower than that for the later, 113mA.
...
level, \eg between 0\% -- 100\%.  We find that {\em brightness level
  adjustment by the hardware button does not change frame pixel values
